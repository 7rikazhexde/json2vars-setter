...
!!! tip "\[*1\]: Specify language"
    ...
    - Target languages: `python`, `nodejs`, `ruby`, `go`, `rust`, `php`, `dotnet`, `java`, `deno`, `bun`, `zig`, `elixir`, `dart`, `swift`, `julia`, `crystal`, `haskell`, `ocaml`, `kotlin`, `clang`, `gcc`
...
| `--languages` | Specify target languages<br>Separate multiple languages with spaces<br>Supported: python, nodejs, ruby, go, rust, php, dotnet, java, deno, bun, zig, elixir, dart, swift, julia, crystal, haskell, ocaml, kotlin, clang, gcc | all |
