...
- **`version/fetchers/`** — Language-specific implementations (`python.py`, `nodejs.py`, `ruby.py`, `go.py`, `rust.py`, `php.py`, `dotnet.py`, `java.py`, `deno.py`, `bun.py`, `zig.py`, `elixir.py`, `dart.py`). Most parse tags from the respective GitHub repository; `java.py` and `dart.py` are exceptions — they override `fetch_versions` to query the Adoptium API and the Dart release archive respectively (see `docs/reference/version-sources.md`)
